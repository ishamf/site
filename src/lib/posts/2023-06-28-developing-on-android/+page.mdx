...
<script>import postInAndroid from './post-in-android.jpg';</script>

I currently have a Samsung S23 Ultra,
which is able to connect to an external display to show a desktop environment
...
There's an app called [Termux](https://termux.dev/) which allows you to open a terminal and run a lot of linux commands. It also has a package manager so you can install more programs.
...
And just like that, I have a (mostly) regular debian instance in my phone. Now I just need to install nodejs inside, and check what's the platform:
...
Seems the code-server install scripts explicitly checks for `--unsafe-perm`.
...
on the github issue. I suspect it's related to the node 18 or my npm version is too new, so this might not affect you.
...
![Writing this entire post in android]({postInAndroid})

## Challenges

There's a few annoying things that I haven't managed to solve yet.

### Browser

Ironically, running the actual developer tools that you need to build and run your app seems to be the easy part. Running VS Code is trickier.

Even after getting `code-server` running, the VS Code UI doesn't work perfectly on mobile browsers. Here's a few issues I found from quickly trying a few browsers:

- Chrome (and a few other chromium-based browsers like Bromite): The mouse hover is not affecting the UI.
...
A physical keyboard and mouse is basically required. There's a few things you can't do without it. You can type, but I haven't found a keyboard where the function keys or `Ctrl-Shift-P` work. Also, you can't use the touchscreen to select text.

### Dev Tools

The android browsers don't have developer tools. I think I can use [Eruda](https://github.com/liriliri/eruda) for that,
but somehow it doesn't work on the DeX browsers (even on its own site).
